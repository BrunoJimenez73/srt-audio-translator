...
class TranslatorEngine:
    ...
    def _run(self):
        ...
        self.log(f"Cargando modelo Faster-Whisper: {self.model_size} (CUDA)")
        try:
            self.model = WhisperModel(self.model_size, device="cuda", compute_type="float16")
            self.log("Modelo cargado exitosamente en GPU.")
        except Exception as e:
            self.log(f"Error cargando CUDA ({e}). Intentando con CPU...")
            try:
                self.model = WhisperModel(self.model_size, device="cpu", compute_type="int8")
                self.log("Modelo cargado en CPU (ADVERTENCIA: Latencia alta).")
